...
- Data sources (`pltf_data_source`, server 3.5.21+): write via `obto_upsert_record` with `script` = `JSON.stringify({collection, pipeline[, label]})` — the server unpacks that JSON into the structured fields the runtime aggregation reads; `{{var}}` placeholders resolve at run time. Writes are policy-gated to admin identities: a permission refusal means the signed-in user lacks the admin role, not that the tool is broken.
