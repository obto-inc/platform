...
**A `pltf_script_server` artifact suddenly looks compiled — comments gone, arrays on one line, `async params =>` style, line count dropped** — on clusters running the compile-on-save engine (app.obto.co, 2026-07+), a successful save stores the Babel OUTPUT in `script` and keeps the submitted source in the record's `ts_source` field. `obto_grep_artifact`/`obto_patch_artifact` address the compiled text, so pre-save line numbers and anchors are dead — re-grep immediately after ANY save and never reuse earlier line numbers. For module-scale edits, edit the SOURCE (local mirror, or `ts_source` read via `obto_db_query`) and re-deploy the full file via `obto_stage_chunk({action:'from_url'})` + sha256; line-patching the compiled text works but widens source/compiled drift, and an insert at a stale line number lands blind (insert mode has no anchor check).
